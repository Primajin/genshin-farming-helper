...
import {levelOptions} from '../constants/levels.js';
...
const getMaterials = (item, currentLevel, targetLevel) => {
	const materials = [];
	if (!item) {
		return materials;
	}
	...
	for (let i = startIndex; i < endIndex; i++) {
		...
		if (costs[levelKey]) {
			for (const material of costs[levelKey]) {
...
function PresetPicker({onClose, onAddPreset}) {
	...
	const handleAddClick = () => {
		...
		if (activeTab === 'fishingRods') {
			const rod = fishingRods.find(rod_ => rod_.name === selectedItem);
			if (rod && rod.sources) {
				materials = rod.sources.map(source => ({name: source, count: 20}));
			}
		} else {
			const items = activeTab === 'characters' ? characters : weapons;
			const item = items.find(i => i.name === selectedItem);
			materials = getMaterials(item, currentLevel, targetLevel);
		}
	...
	return (
		<div css={modalBackdrop} onClick={onClose}>
			<div css={modalContent} onClick={event => event.stopPropagation()}>
				<h2>Preset Picker</h2>
				<div css={tabs}>
					<div key='characters' css={tab(activeTab === 'characters')} onClick={() => handleTabClick('characters')}>Characters</div>
					<div key='weapons' css={tab(activeTab === 'weapons')} onClick={() => handleTabClick('weapons')}>Weapons</div>
					<div key='fishingRods' css={tab(activeTab === 'fishingRods')} onClick={() => handleTabClick('fishingRods')}>Fishing Rods</div>
				</div>
				<div css={tabContent}>
					{renderTabContent()}
				</div>
